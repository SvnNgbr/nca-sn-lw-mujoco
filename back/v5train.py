...
def main():
    model = None
    for stage in STAGES:
        print(f"Training stage {stage['name']} (root_assist={stage['root_assist']}, steps={stage['steps']})")
        ...
        stage_path = MODEL_DIR / f"humanoid_v5_ppo_{stage['name']}"
        model.save(stage_path)
        print(f"Saved stage model to {stage_path}.zip")

    final_path = MODEL_DIR / "humanoid_v5_ppo_curriculum"
