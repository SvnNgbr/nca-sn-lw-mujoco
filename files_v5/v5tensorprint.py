...
def plot_metrics(data, output_dir, run_name, window=20):
    ...
    # Alternative Tag-Namen
    alt_tags = {
        "rollout/ep_rew_mean": ["ep_rew_mean", "reward"],
        "rollout/ep_len_mean": ["ep_len_mean", "episode_length"],
        "train/learning_rate": ["learning_rate"],
        "time/fps": ["fps"],
    }
    
    for idx, (tag, title, ylabel, color) in enumerate(metrics):
        row = idx // 2
        col = idx % 2
        ax = axes[row, col]
        
        # Versuche den Haupt-Tag, dann Alternativen
        found = False
        d = None
