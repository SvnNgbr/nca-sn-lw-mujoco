from pathlib import Path

from stable_baselines3 import PPO
from stable_baselines3.common.monitor import Monitor

from burpee_imitation_env_v2 import BurpeeImitationEnvV2


MODEL_DIR = Path("trained_models")
MODEL_DIR.mkdir(exist_ok=True)


STAGES = [
    {"name": "teacher_strong", "root_assist": 0.98, "steps": 300_000},
    {"name": "teacher_medium", "root_assist": 0.75, "steps": 500_000},
    {"name": "teacher_light", "root_assist": 0.45, "steps": 700_000},
]


def make_env(root_assist):
    return Monitor(
        BurpeeImitationEnvV2(
            model_path="humanoid_burpee_learning.xml",
            random_start=False,
            root_assist=root_assist,
            action_scale=0.45,
        )
    )


def main():
    model = None
    for stage in STAGES:
        print(
            f"Training stage {stage['name']} "
            f"(root_assist={stage['root_assist']}, steps={stage['steps']})"
        )
        env = make_env(stage["root_assist"])
        if model is None:
            model = PPO(
                "MlpPolicy",
                env,
                verbose=1,
                n_steps=2048,
                batch_size=256,
                learning_rate=2e-4,
                gamma=0.98,
                gae_lambda=0.94,
                ent_coef=0.003,
                clip_range=0.2,
                tensorboard_log="runs_curriculum",
            )
        else:
            model.set_env(env)

        model.learn(
            total_timesteps=stage["steps"],
            reset_num_timesteps=False,
            progress_bar=True,
        )
        stage_path = MODEL_DIR / f"burpee_ppo_{stage['name']}"
        model.save(stage_path)
        print(f"Saved stage model to {stage_path}.zip")

    final_path = MODEL_DIR / "burpee_ppo_curriculum"
    model.save(final_path)
    print(f"Saved final curriculum model to {final_path}.zip")


if __name__ == "__main__":
    main()
