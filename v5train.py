...
class BurpeeHumanoidV5Env(gym.Env):
    ...
    def __init__(
        ...
        root_assist=0.9,
        action_scale=0.45,
        hold_seconds=2.0,  #Wie lange muss die Pose gehalten werden?
    ):
        super().__init__()
        ...
        self.hold_seconds = float(hold_seconds)  # NEU
        self.elapsed = 0.0
    ...
    def _get_obs(self):
        ...
        # Hold-Time in Beobachtung
        hold_time_normalized = min(self.phase_hold_time / self.hold_seconds, 1.0)
    ...
    def step(self, action):
        ...
        is_pose_good = pose_error < 0.5  # Schwellwert für "gute" Pose
        
        if current_phase != self.current_phase:
            # Neue Phase -> Timer zurücksetzen
            self.current_phase = current_phase
            self.phase_hold_time = 0.0
            self.pose_held = False
        elif is_pose_good:
            # Pose wird gut gehalten -> Timer erhöhen
    ...
    def _compute_reward(self, action, ctrl):
        ...
        # Bonus für Halten der Pose
        hold_bonus = 0.0
        if self.pose_held:
            hold_bonus = 2.0  # Extra Bonus wenn Pose gehalten wurde
        elif self.phase_hold_time > 0:
            # Proportionale Belohnung für jede gehaltene Sekunde
